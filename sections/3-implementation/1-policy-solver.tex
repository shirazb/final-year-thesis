\section{Optimal Policy Solver with Precise Per-Layer Costs} \label{sec:3-policy-solver}
\sectionmark{Precise Policy Solver}
...
\begin{figure}[t]
    ...
    \caption{Visualisation of traversing all subsequences in `subproblem order'. The horizontal bars represent a subproblem \((i,\, j)\). Traversal order goes from top to bottom. Notice that, whenever an \((i,\, j)\) is encountered, all of its subproblems have already been done.}
...
It would seem we have finished describing the new solver.
...
Now, layers use a precise amount of memory, so when we checkpoint a tensor and recurse to the right, the new memory budget could be any arbitrary number.
This means memory is now a \textit{continuous} state.
As mentioned, this causes a lot of problems.
Primarily, we now have a much larger state space to iterate over in the policy solver, as, no matter what the arbitrary new memory budget is, we need to have solved a subproblem for it.
Thus, a 10GB GPU would result in us having to iterate from \(m\,=\,1\) all the way to 10 billion.
I will show in Section \ref{sec:4-exp-practicality} that this can be dealt with by bucketing memory, and that it only has a reasonable affect on optimality.

Secondly, because layers use a specific amount of memory, the optimal solution to some \(C(i,\,j,\,m)\) may not use all of the \(m\) permitted memory.
...
This new definition of \(C(\cdot)\) of course affects our analysis of peak memory for
\(Q(i,\,j,\,k,\,m)\) in Equation \ref{eqn:3-b-k-1}.
...
\begin{figure}[t]
    ...
    \caption{How peak memory occurs across the three stages of \(Q(\cdot)\). The red cross denotes the memory being freed by the supercall before recursing left.}
...
Also, \(\alpha^f_{N+1}\) and \(\beta^f_{N+1}\) are not actually defined costs as \(f_{N+1}\) does not exist, but since \(\alpha^b_{N+1}\) and \(\beta^b_{N+1}\) are defined, I make the second dimension of their arrays \(N+2\).
I make a similar simplification for the DP tables \(B,\, C,\, D\).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Skipping the Small \texorpdfstring{\(m\)}{\textit{m}}}
...
Thus, the peak memory becomes the max of (i) the max of all the individual backward steps and (ii) for each of those steps, the max of the forward steps to that layer whilst holding the layer's upstream backward.
...
\begin{algorithm}[t]
    \DontPrintSemicolon
    \SetKwData{PeakF}{fpeak\textsubscript{quad}}
    \SetKwData{PeakQuad}{peak\textsubscript{quad}}
    ...
    \For{\(j \;\leftarrow\; i+2\) \KwTo \(N+1\)}{
        \PeakF \(\leftarrow\; \max(\)\PeakF\(,\, (j-2\,=\,i \;?\; 0 \;:\; \beta^f_{j-2})\,+\,\beta^f_{j-1})\)\;
        \PeakQuad \(\leftarrow\; \max(\)\PeakQuad\(,\, \beta^b_j \,+\,\)\PeakF\(,\, \beta^f_{j-1}\,+\,\beta^b_j\,+\,\beta^b_{j-1}\)\;
        \BlankLine
        \ldots\;
    }
    \ldots\;
...
\begin{algorithm}[t]
    \DontPrintSemicolon
    \SetKwData{ComputeF}{fcompute\textsubscript{quad}}
    \SetKwData{ComputeQuad}{compute\textsubscript{quad}}
    ...
    \For{\(j \;\leftarrow\; i+2\) \KwTo \(N+1\)}{
        \ComputeF \(\leftarrow\;\)\ComputeF\(\,+\, \alpha^f_{j-1}\) \;
        \ComputeQuad \(\leftarrow\;\)\ComputeF\(\,+\, \alpha^b_{j-1}\)\;
        \BlankLine
        \ldots\;
    }
    \ldots\;
...
Now we look at the compute costs.
...
Lastly, I will show that, unlike the original policy solver in Algorithm \ref{alg:2-policy-solver}, there is no need to separately initialise \(C(\cdot)\) with the base cases prior to the main loop.

Recall when we discussed the \(j\,=\,i+1\) base case.
...
\begin{algorithm}
    ...
    \SetKwData{PeakL}{peak\textsubscript{L}}
    \SetKwData{Peak}{peak}
    \SetKwData{ComputeFQuad}{fcompute\textsubscript{quad}}
    \SetKwData{ComputeQuad}{compute\textsubscript{quad}}
    \SetKwData{PeakFQuad}{fpeak\textsubscript{quad}}
    \SetKwData{PeakQuad}{peak\textsubscript{quad}}
    \SetKwData{PeakK}{fpeak\textsubscript{k}}
    \SetKwData{ComputeK}{fcompute\textsubscript{k}}
    \SetKw{Continue}{continue}
    \SetKw{Break}{break}

    % hide end for
    \SetKwFor{HFor}{for}{do}{}
    ...
    \# Traverse the subproblems bottom-up.\;
    \HFor{\(m \;\leftarrow\; 2\) \KwTo \(M\)}{
        \HFor{\(i \;\leftarrow\; N\) \KwTo \(0\)}{
            \# Base Case: \([i,\,i+1,\,,m-1]\)\;    

            \(b \;\leftarrow\; \beta^b_i \,+\, \beta^b_{i+1}\)\;
            \(c \;\leftarrow\; \beta^b_i\)\;
            \BlankLine
            
            \If{\(b\;>\;m\)}{
                \(B[i][j][m-1]\;\leftarrow\;\)failure\(, \quad \forall j \,\in\,[i+1,\, N+1]\)\;
                \Continue\;
            }
            \BlankLine

            \(B[i][i+1][m-1]\;\leftarrow\; b\)\;
            \(C[i][i+1][m-1]\;\leftarrow\; c\)\;
            \BlankLine

            \# Recursive Case for each \(j\) to the right.\;
            \BlankLine

            \# Set up memoisation of quadratic strategy costs.\;
            \ComputeFQuad \(\leftarrow\; 0\)\;
            \ComputeQuad \(\leftarrow\;\)\ComputeFQuad\(\,+\, c\)\;
            \BlankLine
            \PeakFQuad \(\leftarrow\; 0\)\;
            \PeakQuad \(\leftarrow\; \max(\)\PeakFQuad\(,\, b)\)\;
            \BlankLine

            \HFor{\(j \;\leftarrow\; i+2\) \KwTo \(N+1\)}{
                \# Update quadratic costs.\;
                \ComputeFQuad \(\leftarrow\;\)\ComputeFQuad\(\,+\, \alpha^f_{j-1}\) \;
                \ComputeQuad \(\leftarrow\;\)\ComputeFQuad\(\,+\, \alpha^b_{j-1}\)\;
                \BlankLine
                \PeakFQuad \(\leftarrow\; \max(\)\PeakFQuad\(,\, (j-2\,=\,i \;?\; 0 \;:\; \beta^f_{j-2})\,+\,\beta^f_{j-1})\)\;
                \PeakQuad \(\leftarrow\; \max(\)\PeakQuad\(,\, \beta^b_j \,+\,\)\PeakFQuad\(,\, \beta^f_{j-1}\,+\,\beta^b_j\,+\,\beta^b_{j-1}\)\;
                \BlankLine

                \# Choose optimal checkpoint \(k\). Initialise variables for its loop.\;
                \PeakK\(\;\leftarrow\; -\infty\)\;
                \ComputeK\(\;\leftarrow\; 0\)\;
                \(C_{min} \;\leftarrow\; \infty\)\;
                \Failed \(\;\leftarrow\; \top\)\;
                % \BlankLine
            }
    ...
    \caption{The final policy solver algorithm for abritrary per-layer compute and memory costs.}
    ...
    \HFor{}{
        \HFor{}{
            \HFor{}{
                \For{\(k \;\leftarrow\; i\,+\,1\) \KwTo \(j\,-\,1\)}{
                    \# Update costs over k.\;
                    \ComputeK\(\;\leftarrow\;\)\ComputeK\(\;+\; \alpha^f_k\)\;
                    \PeakK\(\;\leftarrow\; \max(\)\PeakK\(,\; (k-1\,=\,i \;?\; 0 \;:\; \beta^f_{k-1})\;+\; \beta^b_k)\)\;
                    \BlankLine
                ...
                \If{\Failed}{
                    \If{\PeakQuad\(\;>\; m\)}{
                        \(B[i][j^\prime][m-1]\;\leftarrow\;\)failure\(, \quad \forall j^\prime \,\in\,[j,\, N+1]\)\;
                        \Break\;
                    }
                    \BlankLine
            
                    \(B[i][j][m-1] \;\leftarrow\;\) \PeakQuad\;
                    \(C[i][j][m-1] \;\leftarrow\;\) \ComputeQuad\;
                    \(D[i][j][m-1] \;\leftarrow\;\) quadratic strategy\;
                }
            }
        }
    }
...
\end{algorithm}
\newpage % ensure next section is after algo