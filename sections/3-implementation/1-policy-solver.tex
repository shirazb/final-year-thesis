...
\begin{equation} \label{eqn:3-base-case-comp}
...
This will not be the case as the memory of \(f_i\) was considered in the \(i,\,j\) subproblem.
...
Thus, as the \(j\) loop moves right, we can can memoise the peak of the forward layers separately.
The new overall peak then becomes the max of (i) the peak of the forwards plus this layer's backward and (ii) the backward step for this layer.
...
\begin{algorithm}[htb]
    ...
    \For{\(j \;\leftarrow\; i+2\) \KwTo \(N+1\)}{
        \PeakF \(\leftarrow\; \max(\)\PeakF\(,\, \beta^f_{j-1}\,+\,\beta^f_j)\)\;
...
Equation \ref{3-comp-quad} sums over the layers \(j\).
There are two separate sums for the forwards and backwards costs.
Thought it was easier to explain it in that way, we can rethink it to be more suited to memoisation.

Consider we are at some \((i,\, j)\) subsequence.
Say \(c\) is the memoised cost of executing the quadratic strategy on the subsequence.
As we move right to \(i,\, j+1\), we want to add the marginal cost of performing quadratic on this longer subsequence.
The strategy on \((i,\,j+1)\) does everything the strategy on \(i,\,j\) does, except it has an extra step at the beginning where all the forwards to \(f_j\) are calculated, and then \(b_{j-1}\) is calculated. 
In order to avoid summing the forwards to \(f_j\) every time, we can memoise the cost of the forward pass.
Thus, we memoise both the cost of the forwards and the overall cost of the quadratic strategy.
...
\begin{algorithm}[htb]
    ...
    \For{\(j \;\leftarrow\; i+2\) \KwTo \(N+1\)}{
        \ComputeF \(\leftarrow\;\)\ComputeF\(\,+\, \alpha^f_j\) \;
...
Lastly, I will show that, unlike in the original policy solver in Algorithm \ref{alg:2-policy-solver}, there is no need for a separate initialisation of \(C(\cdot)\) with the base cases, prior to the main loop.
...
Then, only if the recursive case failed to find a viable strategy, do we resort to this `base case'.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The New Policy Solver Algorithm}
Finally, we are done.
